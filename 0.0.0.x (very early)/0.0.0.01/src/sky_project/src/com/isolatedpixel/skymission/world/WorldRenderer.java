...
public class WorldRenderer {
	...
	public WorldRenderer(DemoWorld world) {
		...
		this.world = world;
		this.cam = new OrthographicCamera(12, 9);
		this.cam.position.set(5, 4f, 0);
		this.cam.update();

		bh1 = new Texture(Gdx.files.internal("textures/bh1.png"));
		s = new Sprite(bh1);
		s.setPosition(world.getBob().getBounds().x, world.getBob().getBounds().y);
		s.setSize(world.getBob().getBounds().width, world.getBob().getBounds().height);
	...
	public void Render() {
		...
		batcher.draw(s, bob.getPosition().x, bob.getPosition().y, bob.getBounds().width, bob.getBounds().height);
