...
public class Splash implements Screen {
	...
	public void show() {
		...
		Assets.loadMenus();
		Texture splashTexture = Assets.get(Assets.Splash, Texture.class);
		...
		Tween.set(splashSprite, SpriteAccessor.ALPHA).target(0)
				.start(tweenManager);
		Tween.to(splashSprite, SpriteAccessor.ALPHA, .25f).target(1).start(tweenManager).delay(.2f);
	...
	public void render(float delta) {
		...
		if (Assets.update()) {
			Tween.to(splashSprite, SpriteAccessor.ALPHA, .2f).target(0f).
