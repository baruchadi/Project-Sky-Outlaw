...
//	public boolean checkBob(){
//
//		if(bob != null)
//		return false;
//	
//	}
	public void addBob(Player bob){
		this.bob=bob;
	...
	public void insert(BasicAI rect) {
		...
		if (enemies.size() > MAX_OBJ && lvl < MAX_LVLS) {
			if (nodes[0] == null) {
				split();
			}
			int i = 0;
			while (i < enemies.size()) {
				int index = getIndex(enemies.get(i));
				if (index != -1) {
					nodes[index].insert(enemies.remove(i));
