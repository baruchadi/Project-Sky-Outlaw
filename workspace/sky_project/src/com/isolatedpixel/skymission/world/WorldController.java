...
public class WorldController {
	...
	// update method
	// ********************************************************************************************
	LinkedList<Collideable> returnObj = new LinkedList<Collideable>();
	...
	public void update(float delta) {
		...
		// world.objects.insert(world.wood.get(i));
		// }
		// // world.objects.insert(bob);
		//
		//
		// returnObj.clear();
		// world.objects.retrieve(returnObj, bob);
		//
...
//						world.boxes.add(new Pepper(rand.nextFloat()
//								* (Wood.getSize() - 3) + 2,
//								rand.nextFloat() * 2.5f + .25f));
//					}
	...
	private void processInput() {
		...
		if (keys.get(Keys.DOWN)) {
			...
			if (h) {
				...
				WorldRenderer.testH -= .001f;
			} else {

				WorldRenderer.testY -= .001f;

			}}
			else{
				if (h) {

					WorldRenderer.testW -= .001f;
				} else {

					WorldRenderer.testX -= .001f;
		...
		if (keys.get(Keys.JUMP)) {
			...
			System.out.println("H: "+WorldRenderer.testH+", Y: " +WorldRenderer.testY );
			
				System.out.println("W: "+WorldRenderer.testW+", X: " +WorldRenderer.testX );
			if (bob.g) {// && !bob.crouching
				bob.setVelocity(new Vector2(bob.getVelocity().x, -.235f));
			}
			jumpReleased();
		}
