...
import com.isolatedpixel.skymission.screens.cutscenes.Tavern;
...
public class DifficultySelectorScreen implements Screen {
	...
	public void show() {
		...
		list = new List(new String[] { "Cutscene", "Skip Scene" }, skin);
				...
				stage.addAction(Actions.sequence(Actions.fadeOut(.2f),
						Actions.run(new Runnable() {
							...
							public void run() {
								...
								if (list.getSelection() == list.getItems()[0]) {
									((Game) Gdx.app.getApplicationListener())
											.setScreen(new Tavern());
								} else if (list.getSelection() == list
										.getItems()[1]) {
									((Game) Gdx.app.getApplicationListener())
